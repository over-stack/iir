...
class IIRConv2d(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = F.conv1d(
            x, self.conv1_weight, padding=(self.conv1_window - 1) // 2, groups=channels
        )
        x = F.pad(x, (self.conv2_window, 0), "constant", 0)  # BxCxHxW -> BxCxHx(W+2)
        x = x.transpose(0, 2).contiguous()

        for j in range(self.conv2_window, x.shape[0]):
            x[j] -= torch.einsum(
